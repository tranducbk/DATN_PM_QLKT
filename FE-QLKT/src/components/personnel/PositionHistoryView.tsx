...
export function PositionHistoryView({ role }: PositionHistoryViewProps) {
  ...
  async function loadData() {
    try {
      ...
      if (role === 'admin') {
        baseQueries.push(apiClient.getUnits());
      }

      const results = await Promise.all(baseQueries);
      const [personnelRes, historiesRes, positionsRes] = results;
          ...
          Modal.confirm({
            ...
            onOk: async () => {
              try {
                ...
                const updateRes = await apiClient.updatePositionHistory(historyId!, newPayload);
        ...
        >
          <div>
            <Space style={{ marginBottom: 8 }}>
              <Link href={`${basePath}/personnel/${personnelId}?tab=3`}>
