...
import { Card, Typography, Button, Form, Input, Select, DatePicker, message } from 'antd';
...
interface PersonnelEditFormProps {
  personnelId: string;
  /** Called after a successful save (e.g. navigate back to the detail page). */
  onSuccess: () => void;
...
/**
 * Shared editable form for a personnel record (personal info, address, unit & position).
 * Reused by ADMIN and SUPER_ADMIN edit pages so both share one source of truth.
 * @param personnelId - Id of the personnel to edit
 * @param onSuccess - Invoked after a successful update
 * @param onCancel - Invoked when the user cancels
 */
export function PersonnelEditForm({ personnelId, onSuccess, onCancel }: PersonnelEditFormProps) {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [loadingData, setLoadingData] = useState(true);
  const [coQuanDonViList, setCoQuanDonViList] = useState<UnitOptionRow[]>([]);
  const [donViTrucThuocList, setDonViTrucThuocList] = useState<UnitOptionRow[]>([]);
    ...
    <Card className="shadow-sm">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          >
            <Input placeholder="Nhập số CCCD (tùy chọn)" disabled={loading} maxLength={12} />
          </Form.Item>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {currentUserRole === ROLES.MANAGER ? (
            <Form.Item label="Đơn vị" tooltip="Chỉ Phòng Chính trị mới có thể thay đổi đơn vị">
              ...
              >
                <Select
                  ...
                  onChange={value => {
                    ...
                    setCurrentPositionId(undefined);
                    form.setFieldsValue({
