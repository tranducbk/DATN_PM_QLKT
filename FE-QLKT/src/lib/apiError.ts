...
/**
 * Ghi lỗi API ra console (mọi môi trường) để tra cứu khi người dùng báo lỗi sau.
 * @param error - Giá trị từ `catch`
 * @param context - Mô tả ngắn (vd. tên luồng)
 * @returns void
 */
export function logError(error: unknown, context: string): void {
  console.error(context, getApiErrorMessage(error, 'Lỗi không xác định'));
}
