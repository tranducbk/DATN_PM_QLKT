...
// ADMIN may only manage accounts already at MANAGER/USER level (mirrors backend ADMIN_MANAGED_ROLES).
const canModifyRole = (role: string) => role !== ROLES.SUPER_ADMIN && role !== ROLES.ADMIN;
...
export default function AdminAccountDetailPage() {
  ...
  const canModify = canModifyRole(account.role);
      ...
      <div className="space-y-6 p-6">
        ...
        {!canModify && (
          <Alert
            type="warning"
            showIcon
            message="Không thể chỉnh sửa tài khoản này"
            description="ADMIN chỉ có thể quản lý tài khoản MANAGER và USER."
            className="shadow-sm"
          />
        )}
