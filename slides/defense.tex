...
\begin{frame}{Nội dung trình bày}
  ...
  \vspace{0.15cm}
  \begin{block}{}
    \centering\small Thời lượng trình bày: \highlight{13--14 phút}, sau đó là phần Hỏi \& Đáp với Hội đồng.
  \end{block}
\end{frame}
...
\begin{frame}{Mục tiêu của đồ án}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\small
      \item Quản lý \textbf{5 nhóm khen thưởng} với 4 cấp vai trò
...
% --- Slide 5: Hệ thống khen thưởng — 5 nhóm chính ---
\begin{frame}{Hệ thống khen thưởng --- 5 nhóm chính}
  \scriptsize
  \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{2.6cm}|}
    ...
    \textbf{Khen thưởng hằng năm} & Cá nhân (CSTĐCS, CSTT, BKBQP, CSTĐTQ, BKTTCP) + Đơn vị (ĐVQT, ĐVTT, BKBQP, BKTTCP) & Theo năm \\ \hline
    \textbf{Khen thưởng niên hạn} & HCCSVV (3 hạng), HCQKQT, KNC VSNXD QĐNDVN & Theo thời gian phục vụ \\ \hline
    \textbf{Khen thưởng cống hiến} & HCBVTQ (3 hạng) & Cộng dồn 120 tháng \\ \hline
  ...
  \vspace{0.4cm}
  \begin{block}{}
    Mỗi nhóm được hiện thực bằng một hoặc nhiều \highlight{proposal type} trong code, dispatch qua \highlight{Strategy pattern} (Slide 13).
  \end{block}
\end{frame}
...
\begin{frame}{Thiết kế cơ sở dữ liệu}
  \begin{columns}[T,onlytextwidth]
    ...
    \textbf{Tổng quan}\\
    \begin{block}{}
      \centering {\LARGE \textcolor{hustred}{\textbf{23}}} bảng chính, FK đầy đủ, ID dạng CUID 25 ký tự
    \end{block}
...
\begin{frame}{Thiết kế hoạt động --- Quy trình duyệt đề xuất}
  \begin{columns}[T,onlytextwidth]
    ...
    \footnotesize
    \begin{enumerate}
      \item MANAGER tạo đề xuất qua biểu mẫu năm bước
      \item Hệ thống kiểm tra điều kiện, cảnh báo trùng đề xuất
      \item ADMIN đối chiếu, cấp số quyết định và phê duyệt
    ...
    \begin{block}{Sơ đồ flow đơn giản}
      \scriptsize\centering
      \textbf{MANAGER tạo đề xuất}\\
      $\downarrow$\\
      \textbf{ADMIN duyệt + cấp số QĐ}\\
      $\downarrow$\\
      \textbf{Sinh PDF + lưu}\\
      $\downarrow$\\
...
\begin{frame}[fragile]{Kiến trúc phần mềm --- Strategy Pattern}
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
    \textbf{Vấn đề}\\
    \scriptsize 5 nhóm khen thưởng hiện thực thành 7 proposal type, mỗi loại có logic submit/validate/import khác nhau $\to$ \texttt{if/else} sẽ tạo file 2000+ dòng, khó test và bảo trì.\\[0.3cm]
...
\begin{frame}{Quy trình đề xuất và ra quyết định}
  \begin{columns}[T,onlytextwidth]
    \column{0.55\textwidth}
    \begin{block}{Form đề xuất theo từng nhóm}
      \scriptsize Mỗi nhóm khen thưởng có form riêng. Schema \textbf{Zod chia sẻ giữa client và server} đảm bảo nhất quán.
    \end{block}

    \begin{block}{Workflow đề xuất --- phê duyệt}
      \scriptsize MANAGER tạo đề xuất (cấp đơn vị) $\to$ ADMIN phê duyệt (cấp toàn nghiệp vụ). Admin có thể duyệt hoặc từ chối kèm lý do, gửi thông báo về người đề xuất.
    \end{block}
...
\begin{frame}{Nhập / Xuất dữ liệu hàng loạt qua Excel}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Hỗ trợ}
      \scriptsize Import hàng loạt dữ liệu các nhóm khen thưởng qua Excel + Export báo cáo theo năm và đơn vị.
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{Kiểm thử, Hiệu năng và Triển khai}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{}
      \centering {\LARGE \textcolor{hustred}{\textbf{4}}} kịch bản kiểm thử nghiệp vụ trọng yếu \quad $|$ \quad {\large \textcolor{hustred}{\textbf{N+1 $\to$ 1}}} batch query Excel
    \end{block}
...
\begin{frame}{Kết luận và hướng phát triển}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\scriptsize
      \item Hoàn thành 5 nhóm khen thưởng, 4 vai trò
