...
% Cho phép xuống dòng tên cột DB dài trong bảng (vd: co_quan_don_vi_id)
\newcommand{\dbid}[1]{\nolinkurl{#1}}
\renewcommand{\UrlBreaks}{\do\_\do\.\do\/\do\:\do\-}
...
\hyphenation{
  ...
  Rotation Rotations
  BKBTBQP BKTTCP CSTĐCS CSTĐTQ ĐVQT
...
% \pagestyle{empty} % Header và footer rỗng
\newpage
\pagenumbering{roman} % Đánh số trang dạng La Mã cho phần đầu
\pagestyle{plain} % dùng plain đã customise (page number giữa, không có rule)
