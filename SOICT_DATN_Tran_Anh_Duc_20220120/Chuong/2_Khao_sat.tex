...
\multirow{9}{*}{\parbox{3.2cm}{\centering\textbf{Luồng sự kiện chính}\\\textbf{(Thành công)}}}
 ...
 & 4. & Admin & Nhập số quyết định (file PDF đã được tải lên trước qua màn hình quản lý quyết định, đề xuất tham chiếu qua số quyết định) \\ \cline{2-4}
...
\hline
\textbf{STT} & \textbf{Trường dữ liệu} & \textbf{Mô tả} & \textbf{Bắt buộc} & \textbf{Ví dụ hợp lệ} \\ \hline
1 & Số quyết định & Số quyết định khen thưởng (tham chiếu file PDF đã tải lên trước) & Có & 1234/QĐ-HV \\ \hline
2 & Điều chỉnh hạng & Điều chỉnh hạng cho từng đối tượng nếu cần & Không & Hạng Nhì \\ \hline
3 & Lý do từ chối & Lý do nếu từ chối đề xuất & Khi từ chối & Chưa đủ thâm niên \\ \hline
\end{tabular}
\end{table}
...
\textbf{Kiểm tra điều kiện chuỗi ở hai tầng.} Hàm \texttt{checkChainEligibility} được gọi cả trong hàm tính lại hồ sơ (\texttt{computeEligibilityFlags}) và trong hàm kiểm tra phê duyệt (\texttt{checkAwardEligibility}). Hai tầng phải khớp nhau: nếu có sự khác biệt, một phía sẽ chấp nhận đề xuất mà phía kia hiển thị ``không đủ điều kiện'', gây mất niềm tin của cán bộ vào hệ thống. Để bảo đảm điều này, cả hai hàm dùng chung một danh sách kịch bản kiểm chứng thủ công.
