...
\dbid{co_quan_don_vi_id} & VarChar(30) & FK \mbox{$\rightarrow$ CoQuanDonVi} & Liên kết tới CQĐV \\ \hline
\dbid{don_vi_truc_thuoc_id} & VarChar(30) & FK \mbox{$\rightarrow$ DonViTrucThuoc} & Liên kết tới ĐVTT \\ \hline
...
\dbid{nguoi_duyet_id} & VarChar(30) & \mbox{FK $\rightarrow$ TaiKhoan, NULL} & Admin đã phê duyệt \\ \hline
...
\texttt{nam} & Int & \mbox{UNIQUE (quan\_nhan\_id, nam)} & Năm xét danh hiệu \\ \hline
...
\textbf{Cột} & \textbf{Kiểu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
\texttt{id} & String & PK, CUID & \\ \hline
\dbid{nguoi_thuc_hien_id} & VarChar(30) & \mbox{FK $\rightarrow$ TaiKhoan, NULL} & NULL với hành động hệ thống \\ \hline
...
\textbf{Cột} & \textbf{Kiểu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
\texttt{id} & String & PK, CUID & \\ \hline
\dbid{quan_nhan_id} & VarChar(30) & \mbox{UNIQUE, FK $\rightarrow$ QuanNhan} & Mỗi quân nhân một bản ghi \\ \hline
...
3 & Confirm khi DB thay đổi giữa hai bước & Dữ liệu chen vào sau preview & Transaction rollback, không bản ghi nào được ghi & Đạt \\ \hline
