...
Việc tách lớp Repository khỏi lớp Service được áp dụng từ một lần tái cấu trúc lớn của dự án. Trước đó, Service gọi \texttt{prisma.danhHieuHangNam.findMany(...)} trực tiếp, ràng buộc chặt logic nghiệp vụ với Prisma Client. Sau khi tách, Service gọi qua giao diện \texttt{danhHieuHangNamRepository.findMany(...)}; ranh giới giữa logic nghiệp vụ và truy cập dữ liệu rõ ràng, dễ thay thế ORM trong tương lai mà không phải sửa lại logic.
...
Để minh hoạ cách các tầng phối hợp với nhau trong một module nghiệp vụ cụ thể, phần này trình bày chi tiết module xử lý chuỗi danh hiệu hằng năm cá nhân, là module phức tạp nhất của hệ thống.
...
\begin{enumerate}
    ...
    \item \textbf{Controller} \texttt{profileController.getAnnualProfile} (tại \texttt{controllers/profile.controller.ts}) đọc tham số \mbox{\texttt{personnelId}}, gọi service tương ứng, đóng gói kết quả vào \texttt{ResponseHelper.success}.
    \item \textbf{Service} \texttt{profileService.recalculateAnnualProfile} (tại \texttt{services/profile/annual.ts}) thực thi logic gồm: nạp danh sách \texttt{DanhHieuHangNam} qua repository, gọi \texttt{computeChainContext} để dẫn xuất ngữ cảnh chuỗi, lặp qua \texttt{PERSONAL\_CHAIN\_AWARDS} và gọi \texttt{checkChainEligibility} cho mỗi cấp, ghi kết quả vào bảng suy diễn \texttt{HoSoHangNam}.
    \item \textbf{Eligibility module} \texttt{services/eligibility/chainEligibility.ts} chứa hàm \texttt{checkChainEligibility(config, context, hasReceivedLifetime)}: hàm thuần (pure function), không truy vấn cơ sở dữ liệu, dễ kiểm chứng với mọi bộ đầu vào.
...
Sự tách biệt giữa Service (logic nghiệp vụ) và Eligibility module (quy tắc thuần) là chủ ý: Eligibility module hoàn toàn thuần nên có thể kiểm chứng nhanh với mọi kịch bản đầu vào mà không cần kết nối cơ sở dữ liệu. Service chỉ phụ trách điều phối (nạp dữ liệu, gọi quy tắc, ghi kết quả) nên ranh giới rõ ràng, dễ bảo trì.
...
\textbf{Module xử lý chuỗi danh hiệu.} Trung tâm là interface \texttt{ChainAwardConfig} chứa cấu hình một cấp trong chuỗi (mã danh hiệu, số năm chu kỳ, danh sách cờ tiền điều kiện, có yêu cầu NCKH hay không, có phải danh hiệu một lần duy nhất hay không, tên cột cờ trong cơ sở dữ liệu, nhãn chuỗi). Hai mảng cấu hình \texttt{PERSONAL\_CHAIN\_AWARDS} và \texttt{UNIT\_CHAIN\_AWARDS} chứa danh sách các cấp tương ứng cho cá nhân và đơn vị. Hàm \texttt{checkChainEligibility(config, context, hasReceivedLifetime, flagsInWindow)} đọc cấu hình và đối chiếu với ngữ cảnh thực tế để trả về kết quả \texttt{EligibilityResult} gồm hai trường \texttt{eligible: boolean} và \texttt{reason: string}. Interface \texttt{ChainContext} đóng gói các chỉ số dẫn xuất từ lịch sử (độ dài chuỗi, năm gần nhất nhận từng cấp, số cờ trong cửa sổ trượt).
...
11 & Kiểm tra dữ liệu (BE + FE) & Zod & 4.x \\ \hline
12 & Thông báo realtime & Socket.IO & 4.x \\ \hline
...
Môi trường phát triển sử dụng Visual Studio Code có cài extension cho Prisma, ESLint và Prettier; quản lý mã nguồn qua Git và GitHub; quản lý cơ sở dữ liệu trực quan qua Prisma Studio (lệnh \texttt{npx prisma studio}).
...
Tổng quy mô mã nguồn xấp xỉ 35.000 dòng TypeScript chia đều giữa frontend và backend, cộng với 618 dòng \texttt{schema.prisma}. Kho mã nguồn được quản lý theo monorepo với cấu trúc \texttt{BE-QLKT/} và \texttt{FE-QLKT/} ở mức root.
...
\textbf{Hàm \texttt{computeChainContext} dẫn xuất ngữ cảnh chuỗi.} Đặt tại \texttt{BE-QLKT/src/services/profile/annual.ts}. Hàm nhận đầu vào gồm danh sách bản ghi \texttt{DanhHieuHangNam} của một quân nhân, độ dài chuỗi CSTĐCS hiện tại và năm xét. Đầu ra là đối tượng \texttt{ChainContext} chứa năm bắt đầu chuỗi, năm gần nhất nhận BKBTBQP/CSTĐTQ/BKTTCP nằm trong cửa sổ chuỗi, số năm CSTĐCS đã tích luỹ kể từ lần cuối nhận từng cấp, và số chu kỳ đã bỏ lỡ. Hàm hoàn toàn thuần (không truy vấn cơ sở dữ liệu) nên đã được kiểm chứng thủ công với nhiều bộ dữ liệu mẫu phủ các kịch bản từ chu kỳ vừa đủ tới lỡ nhiều chu kỳ.
...
\textbf{Repository Pattern tách Prisma Client.} Mỗi repository (vd: \texttt{danhHieuHangNamRepository}) đóng gói các thao tác Prisma cụ thể qua các phương thức có tên nghiệp vụ (\texttt{findByPersonnelId}, \texttt{upsertByPersonnelYear}, \texttt{deleteManyByPersonnelId}). Mỗi phương thức nhận thêm tham số \texttt{tx} tuỳ chọn (kiểu \texttt{PrismaLike = prisma | Prisma.TransactionClient}) để có thể tham gia vào transaction do tầng Service mở. Cách thiết kế này cho phép Service mở \texttt{prisma.\$transaction(async tx => ...)} rồi truyền \texttt{tx} qua nhiều lời gọi repository, qua đó bảo đảm hoặc tất cả thao tác cùng thành công, hoặc tất cả cùng huỷ bỏ.

\textbf{Hàm \texttt{checkChainEligibility} thực thi quy tắc chuỗi.} Đặt tại \texttt{BE-QLKT/src/services/eligibility/chainEligibility.ts}. Hàm nhận cấu hình cấp danh hiệu (\texttt{ChainAwardConfig}), ngữ cảnh chuỗi (\texttt{ChainStreaks}), cờ ``đã nhận'' (\texttt{hasReceived}) và bản đồ số cờ trong cửa sổ trượt (\texttt{FlagsInWindow}). Logic ba bước: kiểm tra chặn nhận lại nếu là BKTTCP cá nhân và đã nhận, kiểm tra đủ chu kỳ (\texttt{streakLength} lớn hơn hoặc bằng \texttt{cycleYears} và là bội số của \texttt{cycleYears}), kiểm tra đủ cờ tiền điều kiện trong cửa sổ trượt, kiểm tra đủ thành tích NCKH nếu yêu cầu. Trả về \texttt{EligibilityResult \{ eligible, reason \}} với thông điệp tiếng Việt.
