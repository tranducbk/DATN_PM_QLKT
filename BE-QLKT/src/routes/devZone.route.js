...
const {
  ...
  AWARD_TYPES,
  SYSTEM_FEATURES,
} = require('../constants/devZone.constants');
const { getSetting, setSetting, getSettings } = require('../helpers/settingsHelper');

const DEV_PASSWORD = process.env.DEV_ZONE_PASSWORD || 'admin_msa_!@#';

const ALL_FEATURE_KEYS = [
  ...AWARD_TYPES.map(t => `allow_${t}`),
  ...SYSTEM_FEATURES.map(f => `allow_${f}`),
];
...
const verifyDevPassword = (req, res, next) => {
  const password = req.headers['x-dev-password'] || req.body?.password;
...
router.get('/status', verifyDevPassword, async (_req, res) => {
...
router.post('/cron/trigger', verifyDevPassword, async (_req, res) => {
  try {
...
router.put('/cron/schedule', verifyDevPassword, async (req, res) => {
...
router.put('/features', verifyDevPassword, async (req, res) => {
