...
const { parseCCCD, calculateContinuousCSTDCS } = require('./helpers');
 ...
 */
async function getAllAwards(filters = {}, page = 1, limit = 50) {
  try {
 ...
 */
async function exportAllAwardsExcel(filters = {}) {
  try {
    const { don_vi_id, nam, danh_hieu } = filters;
    const where = {};
    if (nam) where.nam = parseInt(nam);
