...
const { checkDuplicateAward } = require('./validation');
...
async function approveProposal(
  ...
  try {
    ...
    if (proposalType === 'HC_QKQT' && nienHanData && nienHanData.length > 0) {
    ...
    if (proposalType === 'KNC_VSNXD_QDNDVN' && nienHanData && nienHanData.length > 0) {
      const personnelIds = nienHanData.map(item => item.personnel_id).filter(Boolean);
 ...
 */
async function rejectProposal(proposalId, lyDo, adminId) {
  try {
    const proposal = await prisma.bangDeXuat.findUnique({
      where: { id: proposalId },
      include: {
