...
/** Finds a pending proposal whose JSON-array `field` holds an item matching `match`. */
async function findPendingProposalWithItem(
  proposalType: string,
  nam: number,
  field: 'data_danh_hieu' | 'data_nien_han' | 'data_cong_hien',
  ...
  return proposals.find(p => {
    const items = (p[field] as Prisma.JsonArray) || [];
    return (items as Array<Record<string, unknown>>).some(match);
  });
}
...
export async function checkDuplicateAward(
  ...
  if (proposalType === PROPOSAL_TYPES.CA_NHAN_HANG_NAM) {
    ...
    const existing = await findPendingProposalWithItem(
  ...
  if (proposalType === PROPOSAL_TYPES.NIEN_HAN && danhHieu?.startsWith('HCCSVV_')) {
    ...
    const pendingExisting = await findPendingProposalWithItem(
  ...
  if (proposalType === PROPOSAL_TYPES.KNC_VSNXD_QDNDVN) {
    ...
    if (actualAward) {
      return {
        exists: true,
        message: `Quân nhân đã có ${KNC_LABEL} (năm ${actualAward.nam || nam})`,
      };
    }
    ...
    if (pendingExisting) {
      return {
        exists: true,
        message: `Quân nhân đang có đề xuất ${KNC_LABEL} chờ duyệt (năm ${pendingExisting.nam})`,
      };
    }
  }
  ...
  if (proposalType === PROPOSAL_TYPES.CONG_HIEN) {
    ...
    if (pendingExisting) {
      const dataCongHien = (pendingExisting.data_cong_hien as Array<Record<string, unknown>>) || [];
