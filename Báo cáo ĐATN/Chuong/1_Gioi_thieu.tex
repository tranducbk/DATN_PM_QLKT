...
Bên cạnh khen thưởng cá nhân và đơn vị hằng năm, Học viện còn xét nhiều danh hiệu theo thời gian phục vụ hoặc công tác như Huy chương Chiến sĩ vẻ vang (HCCSVV), Huân chương Bảo vệ Tổ quốc (HCBVTQ), Kỷ niệm chương (KNC) và Huy chương Quân kỳ quyết thắng (HCQKQT), mỗi loại có tiêu chuẩn xét riêng. Ngoài ra, thành tích nghiên cứu khoa học (NCKH) được quản lý thành một nhóm riêng và là điều kiện bắt buộc khi xét các danh hiệu cấp cao trong khen thưởng cá nhân hằng năm. Dữ liệu của các nhóm này lại nằm rải rác trong nhiều tệp khác nhau, nên khó kiểm tra và khó giữ được sự nhất quán.

Thứ ba, các tệp dùng chung không có cơ chế ghi lại lịch sử thay đổi.
...
Những hạn chế trên cho thấy cách quản lý thủ công hiện nay không còn phù hợp với công tác khen thưởng tại Học viện, và đó chính là bài toán đồ án hướng tới giải quyết.

\section{Mục tiêu và phạm vi đề tài}
\label{section:1.2}

Trên thị trường có các phần mềm quản lý nhân sự nói chung, nhưng chưa có giải pháp phù hợp với đặc thù nghiệp vụ khen thưởng theo chuỗi điều kiện nhiều năm và cơ cấu tổ chức của Học viện.
...
\begin{enumerate}
    \item \textbf{Quản lý hồ sơ và danh mục cơ bản}: quân nhân, đơn vị theo cơ cấu hai cấp gồm Cơ quan đơn vị và Đơn vị trực thuộc, chức vụ và lịch sử thay đổi chức vụ.
    \item \textbf{Đề xuất và phê duyệt khen thưởng}: tạo đợt đề xuất cho bảy nhóm danh hiệu với bộ lọc hỗ trợ tra cứu nhanh theo đơn vị, năm, giới tính và mốc thời gian phục vụ; quy trình phê duyệt nhiều bước, lưu tệp PDF quyết định kèm số quyết định. Riêng khen thưởng đột xuất phát sinh ngoài chu kỳ và đã có quyết định của cấp trên nên được thêm trực tiếp, không qua bước đề xuất.
    \item \textbf{Tự động hoá nghiệp vụ}: kiểm tra điều kiện chuỗi danh hiệu cho cá nhân (BKBQP, CSTĐTQ, BKTTCP) và đơn vị (BKBQP, BKTTCP), tự động tạo thông báo gợi ý đủ hoặc chưa đủ điều kiện kèm lý do, nhập lịch sử khen thưởng từ Excel theo cơ chế xem trước rồi xác nhận, và xuất danh sách theo nhiều tiêu chí. Quá trình nhập bảo đảm tính toàn vẹn: hoặc ghi nhận toàn bộ, hoặc hoàn tác hoàn toàn.
...
Phạm vi đề tài giới hạn trong các nhiệm vụ trực tiếp phục vụ công tác xét khen thưởng.
...
Dữ liệu được lưu trong PostgreSQL và truy cập qua Prisma ORM. Toàn bộ mô hình dữ liệu được khai báo tập trung trong một tệp, và Prisma dùng khai báo này để đồng bộ cấu trúc cơ sở dữ liệu khi có thay đổi.
...
Các sự kiện phê duyệt, từ chối, xoá đề xuất và nhập khen thưởng qua Excel được thông báo theo thời gian thực qua Socket.IO.
...
Nổi bật là cơ chế xét điều kiện chuỗi danh hiệu thống nhất cho cả cá nhân và đơn vị. Bên cạnh đó, bảy nhóm khen thưởng vốn rời rạc được hợp nhất vào một quy trình chung từ đề xuất, thêm số quyết định đến phê duyệt. Cuối cùng, nhật ký hệ thống ghi lịch sử mọi thao tác làm thay đổi dữ liệu.
...
Chương này đã giới thiệu bối cảnh và những hạn chế của cách quản lý khen thưởng thủ công tại Học viện Khoa học Quân sự, từ đó xác định mục tiêu, phạm vi và định hướng giải pháp công nghệ cho phần mềm.
