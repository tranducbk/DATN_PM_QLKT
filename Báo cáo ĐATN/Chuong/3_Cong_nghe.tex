...
Phiên bản 14 ra mắt tháng 11 năm 2023 đưa App Router lên thành mô hình định tuyến mặc định thay cho Pages Router của các phiên bản trước.
...
Vì vậy Next.js được chọn để tận dụng hệ sinh thái React sẵn có và cơ chế định tuyến, tách mã theo route mà không phải đánh đổi thư viện UI.

\section{TypeScript và Express, nền tảng phía máy chủ}
\label{section:3.2}

Backend của PM QLKT cần một khung HTTP gọn, không áp đặt cấu trúc thư mục cứng nhắc, để tự do bố trí kiến trúc bốn tầng và chèn các bước xác thực, kiểm tra dữ liệu, ghi nhật ký theo đúng nhu cầu nghiệp vụ; Express 4.18 trên nền Node.js 20 LTS đáp ứng tốt yêu cầu đó.
...
Dữ liệu khen thưởng vừa có cấu trúc quan hệ chặt chẽ, vừa cần lưu phần chi tiết biến thiên theo từng loại đề xuất, nên hệ quản trị cơ sở dữ liệu phải mạnh cả về ràng buộc quan hệ lẫn kiểu dữ liệu bán cấu trúc; PostgreSQL 15 đáp ứng đồng thời hai yêu cầu này.
...
Để tận dụng tối đa kiểu tĩnh của TypeScript ở tầng truy cập dữ liệu, đồ án chọn Prisma 6.17 làm ORM; thay vì sinh mã kiểu Active Record, Prisma sinh ra một Client an toàn về kiểu từ một tệp khai báo schema duy nhất.
...
Quy ước đặt tên giữ nguyên tên thực thể tiếng Việt ở cả tầng mã lẫn tầng cơ sở dữ liệu: tên bảng vật lý và tên model dùng \texttt{PascalCase} tiếng Việt (ví dụ \texttt{QuanNhan}, \texttt{DanhHieuHangNam}); tên cột và field dùng \texttt{snake\_case} tiếng Việt (ví dụ \texttt{ho\_ten}, \texttt{ngay\_sinh}).
...
Cột giữ \texttt{snake\_case} đồng nhất với truyền thống đặt tên dữ liệu hành chính tiếng Việt và thuận tiện khi tra cứu trực tiếp trên cơ sở dữ liệu.
Đồ án cũng cân nhắc hai ORM phổ biến khác trên Node.js. Sequelize ra đời sớm và có cộng đồng lớn nhưng hỗ trợ TypeScript chưa tự nhiên, thường phải khai báo kiểu bổ sung. TypeORM dựa nhiều vào decorator trên các lớp thực thể, làm tăng khối lượng khai báo khi mô hình có nhiều quan hệ. Prisma được chọn vì sinh kiểu hoàn toàn tự động từ một tệp schema duy nhất và sinh sẵn truy vấn quan hệ; nhờ đó, với 23 model nhiều quan hệ của PM QLKT, khối lượng mã ở tầng Repository giảm rõ rệt so với truy vấn SQL viết tay. Công cụ migration tích hợp của Prisma cũng giúp theo vết thay đổi schema thuận tiện.
...
Ant Design 5.27 cung cấp sẵn nhiều thành phần React đã hoàn thiện như bảng dữ liệu có lọc và phân trang, biểu mẫu tích hợp xác thực, bộ chọn nhiều cấp và các hộp thoại.
...
Tailwind CSS 3 là khung CSS utility-first cho phép áp các lớp đơn lẻ trực tiếp trong JSX để mô tả bố cục, khoảng cách, màu sắc và đáp ứng nhiều kích cỡ màn hình.
...
Nhu cầu cập nhật tiến độ nhập liệu và đẩy thông báo phê duyệt theo thời gian thực dẫn đến việc dùng Socket.IO 4.8, vốn đóng gói giao thức WebSocket và tự động chuyển sang HTTP long polling khi mạng giữa máy khách và máy chủ không cho phép kết nối WebSocket trực tiếp.
...
Hệ số này cân bằng giữa tốc độ băm mật khẩu và độ khó tấn công vét cạn nếu cơ sở dữ liệu bị rò rỉ.

\section{Zod, kiểm tra hợp lệ dữ liệu hai phía}
\label{section:3.7}

Để tránh duy trì hai cú pháp ràng buộc khác nhau ở hai phía, PM QLKT thống nhất dùng Zod làm thư viện xác thực dữ liệu cho cả backend lẫn frontend, thay vì tách Joi cho backend và Zod cho frontend như nhiều dự án Express truyền thống.
...
Quy trình nhập và xuất danh sách khen thưởng theo tệp Excel đòi hỏi sinh được tệp mẫu có ràng buộc và đọc lại tệp người dùng tải lên, nên đồ án dùng ExcelJS 4.4, thư viện JavaScript cho phép đọc, ghi và thao tác workbook \texttt{.xlsx} phía máy chủ Node.js.
